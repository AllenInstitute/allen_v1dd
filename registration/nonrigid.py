...
def phasecorr_reference(refImg1, ops):
    ...
    Ly,Lx = refImg0.shape
    maskMul = rUtils.spatial_taper(maskSlope, Ly, Lx)

    if ops['1Preg']:
        refImg0 = rUtils.one_photon_preprocess(refImg0[np.newaxis,:,:], ops).squeeze()
    ...
    for n in range(nb):
        ...
        maskMul2 = rUtils.spatial_taper(2 * ops['smooth_sigma'], Ly, Lx)
        ...
        # gaussian filter
        fhg = rUtils.gaussian_fft(ops['smooth_sigma'], cfRefImg.shape[0], cfRefImg.shape[1])
        cfRefImg *= fhg
...
@vectorize(['complex64(float64, float32, float32)', 'complex64(float32, float32, float32)'], nopython=True, target = 'parallel')
...
def phasecorr(data, refAndMasks, ops):
    ...
    if ops['1Preg']:
        X = rUtils.one_photon_preprocess(data.copy().astype(np.float32), ops)
    ...
    Y = rUtils.apply_dotnorm(Y, cfRefImg)
    for n in range(nb):
        for t in range(nimg):
