...
def phasecorr_reference(refImg0, ops):
    ...
    if ops['1Preg']:
        refImg = rUtils.one_photon_preprocess(refImg[np.newaxis,:,:], ops).squeeze()
...
def phasecorr_cpu(data, refAndMasks, lcorr, smooth_sigma_time):
    ...
    X = rUtils.apply_dotnorm(X, cfRefImg)
...
def phasecorr(data, refAndMasks, ops):
    ...
    if ops['1Preg']:
        ...
        X = rUtils.one_photon_preprocess(data.copy().astype(np.float32), ops).astype(np.int32)
