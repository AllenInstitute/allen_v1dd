...
from ..eye_tracking import eye_tracking_processing
...
class OPhysSession:
    ...
    def get_traces(self, plane: int, trace_type: str, valid_only: bool = False, reload: bool = False) -> tuple:
        ...
        if reload or cache_key not in self._trace_cache:
            ...
            # Save in cache
            rois = self.get_rois(plane)
            traces = xr.DataArray(
                trace_data,
                name=trace_type,
                dims=("roi", "time"),
                coords=dict(roi=rois, time=time)
            )
            self._trace_cache[cache_key] = traces
        else:
            # Use cache
            traces = self._trace_cache[cache_key]

        # Valid only filter
        if valid_only:
            return traces.sel(roi=self.is_roi_valid(plane))
        return traces
