...
def get_eye_tracking_df(session: OPhysSession):
    with session.open_file() as nwb_file:
        ...
        eye_data = pd.DataFrame(nwb_group["data"][()])

    eye_data.columns=['cr_center_x', 'cr_center_y', 'cr_width', 'cr_height', 'cr_phi',
...
def get_normalized_pupil_diameter(session: OPhysSession):
    ...
    pupil_width_height = eye_tracking_df[['pupil_width','pupil_height']]
    pupil_ratio_index = np.max(pupil_width_height,axis=1)/np.min(pupil_width_height,axis=1)
    pupil_diameter = np.mean(pupil_width_height,axis=1)
    pupil_diameter[pupil_ratio_index>1.5] = float('nan')
    Q1 = np.nanpercentile(pupil_diameter, 25, method = 'midpoint')
    Q3 = np.nanpercentile(pupil_diameter, 75, method = 'midpoint')
    IQR = Q3 - Q1
    upper = Q3 + 1.5*IQR
    pupil_max = np.min([upper,np.nanmax(pupil_diameter)])
    pupil_diameter_normalized = pupil_diameter/pupil_max

    return xr.DataArray(pupil_diameter_normalized, name="pupil_diameter", coords=dict(time=timestamps))
