...
class DGModelDirectionTuningSizeGain(DGModelBase):
    """
    Models DG responses by fitting a single direction-tuning curve, along with a multiplicative
    gain factor that scales responses to the full-field DG.
    """
    ...
    def __init__(self, **kwargs):
        ...
        for s in range(2):
            for d in range(12):
                ...
                self._all_dg_X[i, -1] = s
                i += 1
        
    def get_trial_label(self, trial_info):
        return trial_info["direction"] + 12*DG_STIM_NAMES.index(trial_info["stim_name"])
    ...
    def _predict(self, X, weights, return_grad=False):
        ...
        n, d = X.shape
        XD, XS = X[:, :12], X[:, -1]
        wD, wS = weights[:12], weights[-1]
        dir_pred = XD.dot(wD)
        gain_scale = (XS*wS + (1-XS))
        y_pred = dir_pred * gain_scale
        
        if return_grad:
            d_ypred_wrt_w = np.empty((d, n))
            d_ypred_wrt_w[:12] = XD.T * gain_scale
            d_ypred_wrt_w[-1] = dir_pred * XS
    ...
    def get_trial_feature_matrix(self, trial_infos, trial_labels):
        ...
        trial_feature_matrix[np.arange(n_trials), -1] = trial_labels // 12 # set size flag
    ...
    def plot_fit(self, axs):
        ...
        # Plot the gain size
        axs[-1].text(0.05, 1, f"SIZE GAIN: {self.weights_[-1]:.2f}", fontsize=12, ha="left", va="top", transform=axs[-1].transAxes)
